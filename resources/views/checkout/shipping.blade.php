    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        async function loadAddress(addressId) {
            try {
                ...
                showToast('Erro', 'Erro ao carregar endereço', 'error');
            }
        }
        ...
        async function deleteAddress(addressId) {
            try {
                ...
                if (data.success) {
                    ...
                    addressCard.remove();
                    closeConfirmModal();
                    showToast('Sucesso', 'Endereço removido com sucesso', 'success');
                } else {
                    showToast('Erro', data.message || 'Erro ao remover endereço', 'error');
                }
            } catch (error) {
        ...
        document.getElementById('addressForm').addEventListener('submit', async function(e) {
            ...
            try {
                ...
                if (data.success) {
                    closeAddressModal();
                    showToast('Sucesso', addressId ? 'Endereço atualizado com sucesso' : 'Endereço cadastrado com sucesso', 'success');
                    ...
                    if (data.errors) {
                        ...
                        Object.values(data.errors).forEach(error => {
                            showToast('Erro', error[0], 'error');
                        });
                    } else {
                        showToast('Erro', data.message || 'Erro ao salvar endereço', 'error');
                    }
                }
            } catch (error) {
                console.error('Erro ao salvar endereço:', error);
                showToast('Erro', 'Erro ao salvar endereço', 'error');
            }
        });
        ...
        document.querySelector('.btn-complete-order').addEventListener('click', async function() {
            ...
            if (!addressId || !shippingMethod) {
                showToast('Atenção', 'Selecione um endereço e método de envio', 'warning');
                return;
            }
            ...
            try {
                ...
                if (data.success) {
                    ...
                    showToast('Erro', data.message || 'Erro ao salvar informações de envio', 'error');
                }
            } catch (error) {
                console.error('Erro:', error);
                showToast('Erro', 'Erro ao processar sua solicitação', 'error');
            } finally {
                hideLoading();
            }
        });
